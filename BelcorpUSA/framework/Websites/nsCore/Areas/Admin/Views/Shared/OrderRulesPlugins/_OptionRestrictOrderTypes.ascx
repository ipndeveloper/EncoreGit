...
<script type="text/javascript">
	<% 
	string radioID = Model.OrderTypeIDs.Count > 0 ? "#restrictOrderTypeY" : "#restrictOrderTypeN";  
	%>
	$(function () { 
...
<div class="pad5 promotionOption restrictOrderTypes">
	...
	<div class="FLabel">
		<label for="ordersRestrict">
			<%=Html.Term("RulesOptions_RestrictOrderTypesOption", "Restrict to Order Types?")%></label>
	</div>
	<div rel="isYes" class="hasPanel" id="orderTypes">
		...
		<div <%= Model.OrderTypeIDs.Count > 0 ? "":"style=\"display: none;\"" %> class="UI-lightBg hiddenPanel pad10 overflow" id="OrderTypes">
			<span class="lawyer">
				<%=Html.Term("RulesOptions_RestrictOrderTypesTip", "Only checked order types will contain the rule.")%>
