...
namespace nsCore.Areas.Accounts.Controllers
{
    public class NotesController : BaseController
    {
        ...
        public virtual ActionResult Add(int? parentEntityID, int? parentId, string subject, string noteText)
        {
            try
            {
                ...
                newNote.NoteTypeID = NetSteps.Data.Entities.Constants.NoteType.AccountNotes.ToInt();
                ...
                var acct = (!parentEntityID.HasValue || CoreContext.CurrentAccount.AccountID == parentEntityID.Value)
                    ? CoreContext.CurrentAccount : Account.Load(parentEntityID.Value);
                acct.SaveNote(newNote);
        ...
        public virtual ActionResult Get(string id, DateTime startDate, DateTime endDate, string searchCriteria)
        {
            try
            {
                ...
                List<Note> notes = Note.LoadByAccountNumber(id).OrderByDescending(n => n.DateCreated).ToList();
        ...
        public virtual string Build(IEnumerable<Note> notes, IEnumerable<Note> allNotes, bool isChild)
        {
            if (notes == null)
                return null;
            ...
            notes.Each((n, i) =>
            {
                ...
                if (!isChild)
                {
                    ...
                    builder.Append("</span>")
                        ...
                        .Append(Translation.GetTerm("PostedBy", "Posted by")).Append(": ").Append(n.UserInfo == null ? Translation.GetTerm("Unknown") : n.UserInfo.Username).Append("</span>").Append(n.NoteText);
