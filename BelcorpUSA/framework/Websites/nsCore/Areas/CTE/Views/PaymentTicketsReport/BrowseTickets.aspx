...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
   <div class="SectionHeader">
		<h2>
			<%=  Html.Term("Browse_Titles", "Browse Titles")%>
        </h2>
	</div>
    <%
        Dictionary<int, string> dcOrderPaymentStatuses = (ViewBag.dcOrderPaymentStatuses as Dictionary<int, string>) ?? new Dictionary<int, string>();
        ...
        Dictionary<int, string> dcForefit = (ViewBag.dcForefit as Dictionary<int, string>) ?? new Dictionary<int, string>();
...
<% Html.PaginatedGrid<NetSteps.Data.Entities.Business.HelperObjects.SearchData.PaymetTycketsReportSearchData>("~/CTE/PaymentTicketsReport/GetPaymentTicketsReport")
.AutoGenerateColumns()
.AddInputFilter(Html.Term("Account", "Account"), "AccountID", (Request.QueryString["AccountId"] == "0") ? "" : Request.QueryString["AccountId"])//startingValue:Convert.ToInt32(Request.QueryString["AccountId"]))
...
.AddInputFilter(Html.Term("TicketNumber", "Ticket Number"), "TicketNumber", (Request.QueryString["TicketNumber"] == "0") ? "" : Request.QueryString["TicketNumber"])
...
.AddSelectFilter(Html.Term("OrderPaymentStatus", "Payment Status"), "OrderPaymentStatusId", dcOrderPaymentStatuses, startingValue: Convert.ToInt32(Request.QueryString["OrderPaymentStatusId"]))
...
.AddSelectFilter(Html.Term("Forefit", "Forefit"), "Forefit", dcForefit, startingValue:-1)

.AddInputFilter(Html.Term("ExpirationDate", "Expiration Date"), "_startDateExpirationDate", (Request.QueryString["ExpirationDateDesde"] != null) ? Request.QueryString["ExpirationDateDesde"] : Html.Term("StarDate", "Star Date"), true)
.AddInputFilter(Html.Term("To", "To"), "_endDateExpirationDate", (Request.QueryString["ExpirationDateHasta"] != null) ? Request.QueryString["ExpirationDateHasta"] : Html.Term("EndDate", "End Date")/*DateTime.Now.ToShortDateString()*/, true, true)

.AddInputFilter(Html.Term("LiquidationDate", "Liquidation Date"), "_startDateLiquidationDate", (Request.QueryString["LiquidationDateInicio"] != null) ? Request.QueryString["LiquidationDateInicio"] : Html.Term("StarDate", "Star Date")/*new DateTime(1900, 1, 1).ToShortDateString()*/, true)
.AddInputFilter(Html.Term("To", "To"), "_endDateLiquidationDate", (Request.QueryString["LiquidationDateFin"] != null) ? Request.QueryString["LiquidationDateFin"] : Html.Term("EndDate", "End Date")/*DateTime.Now.ToShortDateString()*/, true, true)
...
//            });
        ...
        function loadBillingInformation(OrderPaymentID) {
            var url = '<%= ResolveUrl("~/CTE/PaymentTicketsReport/ExportarBoletaRepositorio") %>';
        ...
        function cargarBillingInformation(OrderPaymentID) {
            var url = '<%= ResolveUrl("~/CTE/PaymentTicketsReport/VerificarTipoCuenta") %>';
            // var data = ObtenerSeleccionados();
            var OrderPaymentIDs = new Array();
            OrderPaymentIDs.push(OrderPaymentID);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (BankName != "") {
                        var url = '<%= ResolveUrl("~/CTE/PaymentTicketsReport/ExportarBoleta") %>?';
        ...
        function Exportar()
        {
            ...
            var data =
            {
                    ...
                    pageSize:0,
                    startDateExpirationDate:$("#_startDateExpirationDateInputFilter").val(),
                    endDateExpirationDate:$("#_endDateExpirationDateInputFilter").val(),
                    startDateLiquidationDate:$("#_startDateLiquidationDateInputFilter").val(),
                    endDateLiquidationDate:$("#_endDateLiquidationDateInputFilter").val(),
                    ...
                    Forefit: $("#ForefitSelectFilter").val(),
                    TicketNumber: $("#TicketNumberInputFilter").val(),
                    AccountID: $("#AccountIDInputFilter").val(),
                    OrderPaymentStatusId: $("#OrderPaymentStatusIdSelectFilter").val(),
                    orderBy: orderBy,
        ...
        $(function () {
            ...
            exportToExcel.click(
                 function () {
                     var url = '<%= ResolveUrl("~/CTE/PaymentTicketsReport/ExportarExcelGrillaTicket") %>?';
                     var data = Exportar();
