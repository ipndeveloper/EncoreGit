...
<script type="text/javascript">
    $(function () {
        ...
        function InicializarCampos() {
            ...
            $("#spInitialAmount").text('<%= Model[0].InitialAmount.ToString("N",CoreContext.CurrentCultureInfo) %>');
            $("#spFinancialAmount").text('<%= Model[0].FinancialAmount.ToString("N",CoreContext.CurrentCultureInfo) %>');
            $("#spDiscountedAmount").text('<%= Model[0].DiscountedAmount.ToString("N",CoreContext.CurrentCultureInfo) %>');

            $("#spCurrentExpirationDateUTC").text(FormatDateCulture('<%= Model[0].CurrentExpirationDateUTC %>'));
       
            $("#spDayExpiration").text('<%= Model[0].DayExpiration %>');
            $("#spDayValidate").text(FormatDateCulture('<%= Model[0].DayValidate %>'));
            ...
            if (ViewMethodsRenegotiation == 1 && PaymentCredit == 'S') {
                ...
                var url = '<%= ResolveUrl("~/CTE/PaymentTickets/GeListRenegotiationMethodsByOrder") %>';
        ...
        function FormatDateCulture(date) {
            var date_validate = date.replace(/(\d{2})(\/)(\d{2})/, "$3$2$1");
            var date_convert = new Date(date_validate);
           return $.datepicker.formatDate(format, date_convert);
        ...
        $('#btnRenegotiate').click(function () {
            ...
            if (isComplit) {
                ...
                $.post('<%= ResolveUrl("~/CTE/PaymentTickets/RegisterOrderPayments") %>', data, function (response) {
                    if (response.result) {
    ...
    function ViewShareds(orderPaymentID, renegotiationConfigurationID, negotiationLevelID) {
        ...
        var url = '<%= ResolveUrl("~/CTE/PaymentTickets/GeListRenegotiationShares") %>';
...
//                    $('#trDiscount').style.visibility = "hidden";
       ...
       <a href="<%= ResolveUrl("~/CTE/PaymentTicketsReport/BrowseTickets") %>">
        ...
        <a href="<%= ResolveUrl("~/CTE/PaymentTickets/Renegotiation/" + Model[0].TicketNumber) %>">
