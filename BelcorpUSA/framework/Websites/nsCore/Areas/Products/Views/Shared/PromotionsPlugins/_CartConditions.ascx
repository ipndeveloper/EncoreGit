...
<h3 class="UI-lightBg pad10">
    <%= Html.Term("Promotions_ConditionsCartMustMeetHeading", "Conditions the Cart Must Meet:")%></h3>
<div id="CartConditions">
    ...
    <div id="CriteriaContents">
        ...
        <!-- cart has => subtotal -->
        <% 
            Html.RenderPartial("_CartConditionsSubtotals", Model); 
        %>
        <!-- qv cart subtotal -->
        <% Html.RenderPartial("_CartConditionsQVSubtotals", Model); %>
    </div>
    <input type="hidden" id="HF_URL_1" value="<%=ResolveUrl("~/CartRewardsPromotions/ListProductPriceTypes/")%>" />
</div>
