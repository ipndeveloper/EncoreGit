...
<asp:Content ID="Content0" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#btnSave').click(function () { 
                ...
                if (vTipo == "3") {
                    $("#conditionSingleGrid tbody tr").each(function (index) {
                        ...
                        productsDispatchs.push({ DispatchID: 0, ProductID: campo1, Quantity: campo4, Quantity2: campo5, SKU: "" }); 
                    })
                } else {
                    $("#conditionSingleGrid tbody tr").each(function (index) {
                        var campo1, campo2, campo3, campo4, campo5, campo6;
                        ...
                        productsDispatchs.push({ DispatchID: 0, ProductID: campo1, Quantity: campo4, Quantity2: 0, SKU: "" });
            ...
            function agregarColumna(tableID) {
                var table = document.getElementById(tableID);
                var rowCount = table.rows.length;
            ...
            $('#conditionSingleProductAdd').click(function () {
                ...
                if (productId) {
                    getProductInfo(productId, $(this), function (result) {
                        if (TypeDispatchIDs == 3) {
                            // document.getElementById("sTypeDispatch").disabled = true;
                            $('.ProductIDSel').show();
...
<asp:Content ID="Content1" ContentPlaceHolderID="BreadCrumbContent" runat="server">
    ...
    <%= Html.Term("Products") %></a> > <a href="<%= ResolveUrl("~/Products/Dispatch") %>">
            <%= Html.Term("BrowseDispatch", "Browse Dispatchs")%></a> >
    <%= Html.Term("CreateaNewDispatch", "Create a New Dispatch")%>
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">

    <div class="SectionHeader">
        <h2><%= Html.Term("NewDispatch", "New Dispatch")%>
        </h2>
    </div>
    ...
    <div>
      <table class="FormTable" width="100%">
        <tr>
            ...
            <td colspan="3">
                <input id = "nameDispatch" type="text" class="required pad5 fullWidth"  style="width: 50.182em;" />     
            </td>
        </tr> 
        <tr>
            <td class="FLabel"> 
                ...
                <select id="StartPeriods">
                       ...
                       <%
                            Dictionary<string, string> startperiod = Periods.GetAllPeriods();
                            {
                                ...
                                { %>
                                  <option value="<%=startpair.Key%>"><%=startpair.Key%></option><%
                                }
                            }
                        %>
                    </select>    
                       ...
                       <option value="0">Select Period</option>
                        <%
                            Dictionary<string, string> endperiod = Periods.GetAllPeriods();
                            {
                                ...
                                { %>
                                  <option value="<%=endpair.Key%>"><%=endpair.Key%></option><%
                                }
                            }
                        %>
                    </select>
                      ...
                      <select id="ListDispatch">
                        ...
                        <%
                            var listaDispatchDisplay = Dispatch.listdispatchDisplay();
                                ...
                                { %>
                                  <option value="<%=dispDispatch.DispatchListID%>"><%=dispDispatch.Name%></option><%
                                }
                        %>
                      </select>
                </td>
