...
namespace DistributorBackOffice.Areas.Orders.Controllers
{
    public class EntryPaypalController : OrderEntryController
    {
        ...
        {
            return PartialView("PreparePayment");
        }
        ...
        public ActionResult Pay_Approved(string name, string amount, string orderID, string numCuotas)
        {
            ...
            return PartialView("Pay_Approved");
        }
        ...
        private string ValidateMesageRule()
        {
           ...
           #region "Validar la residual"
           var  existPC = OrderContext.Order.AsOrder().OrderPayments.Where(x => x.BankName == "Product Credit").ToList();
           if (existPC.Count() > 0)
           {
               if (Amount != existPC.ToList().First().Amount)
               {
                   ApplyPaymentPreviosBalance();
                   messageRule = Translation.GetTerm("ValidatePayemtResidualChange", "El monto residual cambio ,Por favor intente deneuvo.");
               }
           }
            #endregion

            #region "Validar orderpayemt( que exista informacion en  paymentsTable y temp)"
            PaymentsTable objE = new PaymentsTable();
            objE.PreOrderID = Convert.ToInt32(Session["PreOrder"]);
            var ct = Order.GetPaymentsTable(objE);
            int numOrdePay = 0;
            ...
            if (Amount > 0 && balancedue >= 0 && Amount >= grandtotal && (ct.Count() != 1 || numOrdePay != 1))
            {
                ApplyPaymentPreviosBalance();
                istotal = false;
                messageRule += Translation.GetTerm("NoPaymentSelect02", "Por favor efetuar o pagamento novamente.");
            }

            if (istotal && (ct.Count() == 0) || (numOrdePay == 0) || (Amount != 0 && balancedue <= 0 && (grandtotal != Amount) && (ct.Count() != 2 || numOrdePay != 2)))
            {
                ApplyPaymentPreviosBalance();
                messageRule += Translation.GetTerm("NoPaymentSelect", "Por favor selecione novamente sua opção de pagamento.");
            }
            #endregion
            ...
            if (OrderContext.Order.AsOrder().OrderPayments.Count == 0 && OrderContext.Order.AsOrder().GrandTotal.Value > 0)
            {
                messageRule +=Translation.GetTerm("NoPaymentsApplied", "No payments applied.");
            }

            if (!OrderContext.Order.AsOrder().OrderCustomers.Any(oc => oc.OrderItems.Count > 0))
            {
                messageRule +=Translation.GetTerm("NoItemsInCart", "There are no items in your cart.");
            }
