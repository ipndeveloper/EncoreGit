    ...
    $.widget('ui.timepickr', {
        _init: function () {
            ...
            element.blur(function (e) {
                ...
                if ((ui.options.checkOnBlur && !/^(0?[1-9]|1[0-2])(:[0-5]\d\s?[aApP][mM])$/.test(element.val())) && ui.options.resetOnBlur) {
        ...
        show: function () {
            ...
            if (this.menu.position().left == 0 && this.element.position().left > 0)
                this._redraw();
            //temp fix for timepickrs going off the screen, probably should find a better solution - DES
            $(this.menu).css('width', '500px');
            this.menu.show().find('ol:first').css('left', this.element.position().left).show();
        },
        ...
        _redraw: function () {
            ...
            if (this.options.convention === 24) {
                ...
                if (this.menu.find('ol:first li:first').hasClass('ui-state-hover')) {
                    // daytime
                    nightHours.hide();
                    dayHours.show();
                } else {
