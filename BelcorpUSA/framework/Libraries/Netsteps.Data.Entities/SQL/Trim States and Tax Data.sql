...
-- TODO: Finish Clearing out Orders Info - JHE
...
DELETE UserRoles WHERE UserID IN (SELECT UserID FROM Users WHERE Username <> 'Admin')
DELETE Users WHERE Username <> 'Admin'
DBCC CHECKIDENT (CorporateUsers, reseed, 1)
DBCC CHECKIDENT (Users, reseed, 1)


-- Clean Some Data
UPDATE StateProvinces SET Name =  LTRIM(RTRIM(Name))
UPDATE StateProvinces SET StateAbbreviation =  LTRIM(RTRIM(StateAbbreviation))

UPDATE TaxCache SET State =  LTRIM(RTRIM(State))
UPDATE TaxCache SET StateAbbreviation =  LTRIM(RTRIM(StateAbbreviation))
UPDATE TaxCache SET City =  LTRIM(RTRIM(City))
UPDATE TaxCache SET County =  LTRIM(RTRIM(County))

UPDATE TermTranslations SET TermName =  LTRIM(RTRIM(TermName))
