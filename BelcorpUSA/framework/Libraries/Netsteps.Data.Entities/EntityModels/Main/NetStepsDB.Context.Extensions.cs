...
namespace NetSteps.Data.Entities
{
    public static class SelfTrackingEntitiesContextExtensions
    {
        ...
        // This method has been customized for nullable keys.
    	// http://social.msdn.microsoft.com/Forums/en-US/adodotnetentityframework/thread/0d0582fd-9241-49e3-82a0-e6c2530bf0ab/#31947301-cc32-40bc-8aa1-28c0da07d91d
        private static void UpdateOriginalValues(ObjectContext context, IObjectWithChangeTracker entity)
        {
            ...
            var primitives = (from p in entityType.Properties
                              join ov in entity.ChangeTracker.OriginalValues on p.Name equals ov.Key
                              join fm in currentValues.DataRecordInfo.FieldMetadata on p.Name equals fm.FieldType.Name
                              where p.TypeUsage.EdmType is PrimitiveType
                              select new
                              {
                                  Ordinal = fm.Ordinal,
                                  Nullable = p.Nullable,
                                  CurrentValue = currentValues[fm.Ordinal],
                                  OriginalValue = ov.Value,
                                  Property = p
                              }).ToList();
            var complex = from p in entityType.Properties
                          where p.TypeUsage.EdmType is ComplexType
                          select p;
            //
            // Deal with null original values.
            // Since we cannot set an original value to null, remember the current value,
            // and then set the current value to null.  Apply this change to the original value,
            // and set the current value back to what we started with.
            //
            primitives.ForEach(a =>
                {
                    if (a.OriginalValue == null && a.Nullable)
                        currentValues.SetDBNull(a.Ordinal);
                    else
                        currentValues.SetValue(a.Ordinal, a.OriginalValue);
                });
            entry.ApplyOriginalValues(entity);
            primitives.ForEach(a => currentValues.SetValue(a.Ordinal, a.CurrentValue));
            // Complex Types
            complex.ToList().ForEach(p =>
                {
                    OriginalValueRecord complexOriginalValues = originalValueRecord.GetOriginalValueRecord(p.Name);
                    UpdateOriginalValues((ComplexType)p.TypeUsage.EdmType, entity.GetType().FullName, p.Name, entity.ChangeTracker.OriginalValues, complexOriginalValues);
                });
    		// ***END CUSTOM CODE***
        }
