...
{
    public partial class QueueItemPriority : EntityBusinessBase<QueueItemPriority, Int16, IQueueItemPriorityRepository, IQueueItemPriorityBusinessLogic>, IDatabaseValidationRules, ITermName, IActive
    {
    	...
    	#region Constructors
    	public QueueItemPriority() : this(default(IQueueItemPriorityRepository), default(IQueueItemPriorityBusinessLogic))
    	{
    	}
    	public QueueItemPriority(IQueueItemPriorityRepository repo) : this(repo, default(IQueueItemPriorityBusinessLogic))
    	{
    	}
    	public QueueItemPriority(IQueueItemPriorityRepository repo, IQueueItemPriorityBusinessLogic logic)
    		: base(repo, logic)
    	{
    		InitializeEntity();
    	}
    	...
    	public static List<QueueItemPriority> LoadAll()
    	{
    		try
    		{
    	...
    	// Generated for the "small collection" tables. - JHE
    	public static List<QueueItemPriority> LoadAllFull()
    	{
    		try
    		{
    	...
    	public static SqlUpdatableList<QueueItemPriority> LoadAllFullWithSqlDependency()
    	{
    		try
    		{
    			var queueItemPriorities = Repository.LoadAllFullWithSqlDependency();
    			foreach (var queueItemPriority in queueItemPriorities)
    			{
    				queueItemPriority.StartTracking();
    				queueItemPriority.IsLazyLoadingEnabled = true;
    			}
    			return queueItemPriorities;
    		}
    		catch (Exception ex)
    		{
    			throw EntityExceptionHelper.GetAndLogNetStepsException(ex, Constants.NetStepsExceptionType.NetStepsBusinessException);
