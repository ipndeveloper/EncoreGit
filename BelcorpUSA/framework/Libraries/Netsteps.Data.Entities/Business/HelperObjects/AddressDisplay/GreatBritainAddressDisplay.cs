...
{
    public class GreatBritainAddressDisplay : BaseAddressDisplay
    {
        public override bool IsMatch(string cultureInfoCode)
        {
            return cultureInfoCode.EndsWith("GB", StringComparison.OrdinalIgnoreCase);
        }
        ...
        public override void DisplayFormat(StringBuilder builder, AddressDisplayModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.Address.County))
                builder.Append(model.Address.County).Append(model.Delimiter);
            if (!string.IsNullOrEmpty(model.Address.PostalCode))
                builder.Append(model.Address.PostalCode);
        }
    }
}
