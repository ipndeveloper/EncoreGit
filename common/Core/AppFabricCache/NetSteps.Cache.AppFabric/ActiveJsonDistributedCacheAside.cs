...
{
	public sealed class ActiveJsonDistributedCacheAside<K, R> : JsonDistributedCacheAside<K, R>
		where R : class
	{
		#region Fields

		static readonly ILogSink __log = typeof(ActiveJsonDistributedCacheAside<K, R>).GetLogSink();
		...
		public ActiveJsonDistributedCacheAside(string cacheName, ICacheItemResolver<K, R> resolver)
			: this(cacheName, resolver, null)
		{ }

		public ActiveJsonDistributedCacheAside(string cacheName, ICacheItemResolver<K, R> resolver, MruCacheOptions options)
