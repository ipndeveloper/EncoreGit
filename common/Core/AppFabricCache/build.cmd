...
ECHO.%~p1
ECHO. %~1
SET "CL=msbuild %F% /p:Configuration=%CFG%;Platform=%PLT%;RestorePackages=true /v:%VRB% > build_%UNIQUE%.log /m"
IF %BLD% NEQ "" (
	if %TEN% NEQ "" (
		SET "CL=msbuild %F% /t:Clean,Build /p:Configuration=Release;Platform=%PLT%;RestorePackages=true;CMBuild=true;MvcBuildViews=false /v:%VRB% > build_%UNIQUE%.log /m"
	)
)
%CL%
IF %ERRORLEVEL% NEQ 0 (
