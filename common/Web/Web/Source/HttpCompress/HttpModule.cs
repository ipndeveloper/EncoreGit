...
namespace NetSteps.Web.HttpCompress
{
	/// <summary>
	...
	public class HttpModule : IHttpModule
	{
		...
		void context_PostReleaseRequestState(object sender, EventArgs e)
		{
			try
			{
				...
				{
					settings = (Configuration)ConfigurationManager.GetSection("NetStepsWeb/HttpCompress");
		...
		private class CompressionPageFilter : Stream
		{
			...
			public string ReplaceCss(string html)
			{
				...
				foreach (Match match in Regex.Matches(html, _cssPattern, RegexOptions.IgnoreCase))
				{
					...
					// Added to skip FavIcon entries (image/x-icon) - JHE
					if (!Regex.IsMatch(linkHtml, "type=[\\\"\\']?text/css[\\\"\\']?", RegexOptions.IgnoreCase))
						continue;

					// Added because it seem an external URL doesn't work - JHE
					if (linkHtml.ToLower().Contains("http://jqueryui.com"))
						continue;

					if (linkHtml.ToLower().Contains("WebResource.axd".ToLower()))
						continue;

					if (linkHtml.ToLower().Contains("ScriptResource.axd".ToLower()))
						continue;

					if (linkHtml.ToLower().Contains("main-IE".ToLower()))
						continue;

					// Custom code to overload the relative path issue with the "App_Themes" directory - JHE
					if (href.ToUpper().Contains("APP_THEMES"))
					{
						int startIndex = href.IndexOf("App_Themes/");
						if (startIndex < href.Length)
							href = VirtualPathUtility.ToAbsolute("~/" + href.Substring(startIndex));
					}
