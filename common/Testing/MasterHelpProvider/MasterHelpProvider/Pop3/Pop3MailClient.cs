...
namespace Utilities.Email.Pop3
{
    ...
    #region " Delegates for Pop3MailClient "
    ...
    public class Pop3MailClient
    {
        ...
        public void Connect()
        {
            if (pop3ConnectionState != Pop3ConnectionStateEnum.Disconnected &&
              ...
              !isTimeoutReconnect)
            {
                CallWarning("connect", String.Empty, "Connect command received, but connection state is: " + pop3ConnectionState.ToString());
            }
            else
            {
        ...
        public void Disconnect()
        {
            ...
            pop3ConnectionState == Pop3ConnectionStateEnum.Closed)
            {
                CallWarning("disconnect", String.Empty, "Disconnect received, but was already disconnected.");
            }
            else
            {
        ...
        public List<Message> GetMessageList()
        {
            ...
            if (response.Contains("+OK 0 messages"))
            {
                return ReturnArray;
            }
            ...
            while (readMultiLine(out response))
            {
                ...
                if (Values.Length > 1)
                {
                    ...
                    TempMessage.MessageNumber = Int32.Parse(Values[0]);
                    TempMessage.MessageSize = Int32.Parse(Values[1]);
        ...
        public Message GetMessage(Message MessageWanted)
        {
            ...
            if (!SendRetrCommand(Convert.ToInt32(MessageWanted.MessageNumber)))
            {
                BodyText = null;
            }
            ...
            BodyText = RawEmailSB.ToString();
            TraceFrom("email with {0} lines,  {1} chars received", LineCounter.ToString(), BodyText.Length);

            TempMessage.MessageBody = new Utilities.Email.MIME.MIMEMessage(BodyText);
        ...
        private bool executeReconnect(IOException ex, string command, byte[] commandBytes)
        {
            if (ex.InnerException != null && ex.InnerException is SocketException)
            {
                ...
                if (innerEx.ErrorCode == 10053)
                {
                    ...
                    CallWarning("ExecuteCommand", String.Empty, "probably timeout occured");
                    if (isAutoReconnect)
                    {
        ...
        protected void SendCommand(string command) {
          ...
          try {
            ...
            if (ex.InnerException!=null && ex.InnerException is SocketException) {
              ...
              if (innerEx.ErrorCode==10053) {
                //probably timeout: An established connection was aborted by the software in your host machine.
                CallWarning("SendCommand", String.Empty, "probably timeout occured");
                if (isAutoReconnect) {
                  //try to reconnect and send one more time
                  isTimeoutReconnect = true;
                  try {
