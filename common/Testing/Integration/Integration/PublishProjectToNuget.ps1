param(
    [Parameter(Mandatory=$true)]$nugetKey,
    $majorVersion = 1,
    $minorVersion = 0,
	$preReleaseString = '',
    ...
    $domainUserName = "",
    $domainPassword = "",
    $projectPath = 'C:\Development\NetSteps\Components\Testing\Core\Testing.Core.csproj',
...
if($mode -eq "SVN"){
    ...
    & $msbuildPath `
        $projectPath `
        '/t:Clean;Build;PackageForNuget;PublishToNetStepsNuget' `
