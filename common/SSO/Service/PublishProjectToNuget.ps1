param(
    ...
    $domainUserName = "",
    $domainPassword = "",
    $projectPath = 'C:\Development\NetSteps\Components\SSO\Service\NetSteps.SSO.Service.csproj',
...
if($mode -eq "TFS"){
    ...
    $nugetProperties = `
        "SSOCommonVersion=" `
        + (Get-Command C:\Development\NetSteps\Components\SSO\Service\bin\Release\NetSteps.SSO.Common.dll).FileVersionInfo.ProductVersion
