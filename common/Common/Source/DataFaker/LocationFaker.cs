...
namespace NetSteps.Common.DataFaker
{
	...
	public static class LocationFaker
	{
		...
		{
			return _countries.GetRandom().ToCleanString();
		}

		static public string State()
		{
			return _states.GetRandom().ToCleanString();
		}

		static public string City()
		{
			return _cities.GetRandom().ToCleanString();
		}
		...
		static public string StreetName()
		{
			...
			string place = streetnames.GetRandom().ToCleanString();
			...
			if (Random.GetBoolean())
			{
				...
				string tree = trees.GetRandom().ToCleanString();
			...
			else
			{
				...
				string person = people.GetRandom().ToCleanString();

				result = person + " " + place;
			}

			List<string> streetTypes = "Way, Drive, Circle, Avenue, Street, Road, Lane".ToStringList();
			result = result + " " + streetTypes.GetRandom().ToCleanString();
