...
{
    public class OrderCustomer_CommissionableTotal_ValidationHandler : BaseRecordPropertyCalculationHandler
    {
        public OrderCustomer_CommissionableTotal_ValidationHandler(IRecordPropertyCalculationHandlerResolver resolver)
            : base(resolver)
        {
        }
        ...
        public override void CalculateExpectedValue(Validation.Common.Model.IRecordProperty propertyToCalculate)
        {
            ...
            var primaryVolumePriceType = ServiceLocator.FindService<IPriceTypeService>().GetPrimaryVolumeType();
            ...
            {
                var productPriceTypeID = primaryVolumePriceType.PriceTypeID;
            ...
            {
                propertyToCalculate.ParentRecord.AddValidationComment(ValidationCommentKind.CalculationComment, String.Format("OrderCustomer CommissionableTotal was {0}. Should be {1} but falls within the margin of error {2}.", propertyToCalculate.OriginalValue, propertyToCalculate.ExpectedValue, maximumAllowedDeviancy));
                propertyToCalculate.SetResult(ValidationResultKind.IsWithinMarginOfError);
            }
            else
            {
                propertyToCalculate.ParentRecord.AddValidationComment(ValidationCommentKind.CalculationComment, String.Format("OrderCustomer CommissionableTotal was {0}. Should be {1}.", propertyToCalculate.OriginalValue, propertyToCalculate.ExpectedValue));
