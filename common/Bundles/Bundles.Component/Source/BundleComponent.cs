...
namespace NetSteps.Bundles.Component
{
	...
	[ContainerRegister(typeof(IBundleComponent), RegistrationBehaviors.DefaultOrOverrideDefault, ScopeBehavior = ScopeBehavior.Singleton)]
	public class BundleComponent : IBundleComponent
	{
		...
		public IEnumerable<ICustomerKitResult> AutoBundleItems(IBundleOrder order, int storeFrontID)
		{
			...
			List<ICustomerKitResult> returnValue = new List<ICustomerKitResult>();

			foreach (var customer in order.OrderCustomers)
			{
				List<IDynamicKit> kitsCustomerCanQualifyFor = new List<IDynamicKit>();
				foreach (var kitProduct in sortedDynamicKitProducts)
				{
					...
					{
						if (!kitsCustomerCanQualifyFor.Any(kccqf => kccqf.DynamicKitID == dynamicKit.DynamicKitID)
