...
namespace NetSteps.Sql.CacheNotifications.Buffers
{
	...
	public abstract class BufferWriter : IBufferWriter
	{
		...
		public void FillBytes(byte[] buffer, ref int offset, int count, byte value)
		{
			...
			{
				throw new ArgumentOutOfRangeException("count", "buffer, offset and count paramaters would result in a Buffer Overrun.");
			}
		...
		public int Write(byte[] buffer, ref int offset, byte[] value)
		{
			...
			{
				throw new ArgumentOutOfRangeException("offset", "buffer, offset and value.Length paramaters would result in a Buffer Overrun.");
			}
		...
		public int Write(byte[] buffer, ref int offset, string value, bool byteLengthPrefix, Encoding coder)
		{
			...
			if (byteLengthPrefix)
			{
				if (((buffer.Length - offset) + sizeof(Int32)) < encodedLength)
					throw new InvalidOperationException("offset paramater would result in a Buffer Overrun.");
